...
class PhoneDecoder:
    ...
    def compute(self, logits, lang_id=None, topk=1, emit=1.0, timestamp=False):
        ...
        :return:
        """
        # print("#### Logits are of type {} with shape {}".format(type(logits), logits.shape), flush=True) # n8hack
        # print("#### lang_id:{}, topk:{}, emit:{}, timestamp:{}".format(lang_id, topk, emit, timestamp), flush=True) # n8hack
        ...
        for i in range(len(logits)):
            ...
            if arg_max != cur_max_arg and arg_max != 0:
                ...
                cur_max_arg = arg_max
        # print("#### Emit frame indices:", emit_frame_idx, flush=True) # n8hack

        # decode all emitting frames
        decoded_seq = []
        just_all_phones_probs = [] # n8hack
        for idx in emit_frame_idx:
            ...
            if topk == 1:
                ...
                just_phones_probs = [(phone, prob) for phone, prob in zip(mask.get_units(top_phones), top_probs)] # n8hack
                ...
                decoded_seq.append(phones_str)
                just_all_phones_probs.append(just_phones_probs) # n8hack
        ...
        return just_all_phones_probs #phones # n8hack
